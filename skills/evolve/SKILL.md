...
3. the default verify command scores capability dimensions plus completeness bonuses for fully-covered lanes. use the live gap snapshot, not a stale assumption, to choose the next lane.
...
8. if the current live capability snapshot materially disagrees with older journal or results entries, trust the live snapshot and treat the lower number as historical regression context.
9. if the same benchmark gap fails to produce a kept gain for 2 sessions, stop retrying the same direct fix. either fix the blocker that broke verification or pivot to the next highest-gap capability.
10. treat repeated crash loops as a harness problem first, not proof that the original capability idea is still the best next task.
11. if git is stalled, prefer a safer open non-git lane that still moves the metric, especially navigation or search when they are one counted test short of cap.
12. if you work on git, use temporary repos or other isolated sandboxes only. never mutate the project repo to exercise git behavior.
